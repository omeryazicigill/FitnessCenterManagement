    ...
    public class AppointmentController : Controller
    {
        ...
        public async Task<IActionResult> Create(AppointmentViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                // Check for conflicting appointments (trainer)
                var hasTrainerConflict = await _context.Appointments
                ...
                // Check for conflicting appointments (user - same user cannot book multiple trainers at the same time)
                var hasUserConflict = await _context.Appointments
                ...
                if (hasUserConflict)
                {
                    ModelState.AddModelError("", "Bu saat diliminde zaten başka bir randevunuz bulunmaktadır.");
                    return await ReloadCreateView(model);
                }
        ...
        public async Task<IActionResult> GetAvailableTimes(int trainerId, DateTime date)
        {
            ...
            // Get existing appointments for the trainer on that day
            ...
            // Get existing appointments for the user on that day (user cannot book multiple trainers at the same time)
            ...
            while (currentTime <= endTime)
            {
                ...
                // Check if slot conflicts with trainer's existing appointments
                var isTrainerSlotTaken = trainerAppointments.Any(a =>
                    (currentTime >= a.StartTime && currentTime < a.EndTime) ||
                    (slotEnd > a.StartTime && slotEnd <= a.EndTime));

                // Check if slot conflicts with user's existing appointments
                var isUserSlotTaken = userAppointments.Any(a =>
                    (currentTime >= a.StartTime && currentTime < a.EndTime) ||
                    (slotEnd > a.StartTime && slotEnd <= a.EndTime));

                if (!isTrainerSlotTaken && !isUserSlotTaken && (date.Date > DateTime.Today || 
